...
let mensaje = ""
elBoss.reto[0][0].sigueElReto[0].sigueElReto2.sigueElReto3.pareceQueSabesBucearEntreObjetos.definitivamenteSabes.forEach(item => {
  typeof item === "string" && (mensaje += ` ${item}`)
})
console.log(mensaje)

// Por último, como bonus, mostraremos por pantalla las urls de todas las imágenes de los poderes.
elBoss.poderes.forEach(poder => {
  console.log(`las imagines de tu poder ${poder.nombre} son:`)
  poder.imagenes.forEach(img => {
